...
class SimpleMovingAverages(object):
    ...
    def _calc(self, period, price_source):
        ...
        if price_source in self.ohlcv_df.columns:
...
class ExponentialMovingAverages(object):
    ...
    def _calc(self, period):
        ...
        if 'Close' in self.ohlcv_df.columns:
...
class RSI(object):
    ...
    def run(self):
        ...
        price_dif = self.ohlcv_df['Close'].diff(1)
        gains = price_dif.where(price_dif > 0, 0)
        losses = -price_dif.where(price_dif < 0, 0)
...
class VWAP(object):
    ...
    def run(self):
        #calc vwap
        #find the cummulative sum of volume
        vol = self.ohlcv_df['Volume'].cumsum()
        #avg of high low close
        price = (self.ohlcv_df['High'] + self.ohlcv_df['Low'] + self.ohlcv_df['Close']) / 3.0
        self.vwap = (price * vol) / vol
